...
def get_nt_size(record_dict):
    ...
    """
    return sum(len(seq) for seq in record_dict.itervalues())
...
# Parsing BLAST outputs
    blast_cols = ['quid', 'suid', 'iden', 'alen', 'mism', 'gapo', 'qsta', 'qend', 'ssta', 'send', 'e_val', 'bits']
    recruited_mg=[]
    for i in range(len(mg_lst)):
        try:
...
#            print key, len(value)
    ...
    for i in range(len(mg_lst)):
        start = time.time()
        #TODO:
        rec_dct = parse_contigs_ind(mg_lst[i])
        all_records.append(rec_dct)
        all_sizes.append(get_nt_size(rec_dct))
...
# Transforming data
    ...
    #cutoff_contigs[dataframe]=evalue_filter(cutoff_contigs[dataframe])
        ...
        try:
            ...
            recruited_mg[i]['Read_RPKM_wt'] = recruited_mg[i]['alen'] / (
                        (recruited_mg[i]['Ref_size'] / 1000.0) * (all_sizes[i] / 1000000.0))
        except:
            recruited_mg[i]['Read_RPKM']=np.nan
        
        #recruited_mg[i] = recruited_mg[i][['quid', 'suid', 'iden', 'alen','Coverage','Metric', 'mism', 'gapo', 'qsta', 'qend', 'ssta', 'send', 'e_val', 'bits','Ref_size','Ref_GC','Ref_nt','Contig_size','Contig_GC','Contig_nt']]
        recruited_mg[i] = recruited_mg[i][['quid', 'suid', 'iden', 'alen','Coverage','Metric', 'mism', 'gapo', 'qsta', 'qend', 'ssta', 'send', 'e_val', 'bits','Ref_size','Ref_GC','Contig_size','Contig_GC','Read_RPKM', 'Read_RPKM_wt', 'Contig_nt']]
...
# Quality filtering before outputting
    if alen_percent:
        for i in range(len(recruited_mg)):
            recruited_mg[i]=recruited_mg[i][(recruited_mg[i]['iden']>=iden)&(recruited_mg[i]['Coverage']>=alen)&(recruited_mg[i]['e_val']<=e_val)]
    if alen_bp:
        for i in range(len(recruited_mg)):
            recruited_mg[i]=recruited_mg[i][(recruited_mg[i]['iden']>=iden)&(recruited_mg[i]['alen']>=alen)&(recruited_mg[i]['e_val']<=e_val)]
...
#Writing logfile
        ...
        #this part added for weighted rpkm
        rpkm_wt_lst = np.array(recruited_mg[i]['Read_RPKM_wt'].tolist())
        if(len(rpkm_wt_lst) > 0):
            rpkm_wt = np.sum(rpkm_wt_lst)
            rpkm_wt_std= np.std(rpkm_wt_lst)
            rpkm_wt_sem = np.std(rpkm_wt_lst)*np.sqrt(len(rpkm_wt_lst))
...
#recruited_mg_1.columns=['quid', 'suid', 'iden', 'alen', 'mism', 'gapo', 'qsta', 'qend', 'ssta', 'send', 'e_val', 'bits']

#recruited_mg_2 = pandas.read_csv(out_name2 ,sep="\t", header=None)
#recruited_mg_2.columns=['quid', 'suid', 'iden', 'alen', 'mism', 'gapo', 'qsta', 'qend', 'ssta', 'send', 'e_val', 'bits']
